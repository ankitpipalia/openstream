...
use lowlat_core::envelope::{Direction, Envelope};
...
fn endpoint(arena: &mut Arena) -> Session<'_> {
    endpoint_directed(arena, Direction::Host)
}

fn endpoint_guest(arena: &mut Arena) -> Session<'_> {
    endpoint_directed(arena, Direction::Guest)
}

fn endpoint_directed(arena: &mut Arena, direction: Direction) -> Session<'_> {
    let mut session = Session::with_direction(Envelope::from_key(&KEY).unwrap(), direction, 1, 0.0);
    session
        .attach_recv(
            CHANNEL,
...
fn transfer(seed: u64, link: Link) -> f64 {
    ...
    let mut rx = endpoint_guest(&mut rx_arena);
