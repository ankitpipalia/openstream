...
fn shell<'a>(
    ...
    shell_directed(arena, ours, theirs, seed, Direction::Host)
}
...
fn shell_guest<'a>(
    ...
    shell_directed(arena, ours, theirs, seed, Direction::Guest)
}
...
fn a_pass_larger_than_the_staging_batch_still_completes() {
    ...
    thread::spawn(move || {
        ...
        let mut right = shell_guest(&mut right_arena, RIGHT, LEFT, 0xD4);
