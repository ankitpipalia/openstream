...
fn main() {
    eprintln!("lowlatd host service is currently implemented for Linux");
}
...
mod linux {
    ...
    use lowlat_kessel::message::{
        Answer, AnswerData, CancelRelay, Candex, CandexRelay, CandidateData, ConnUpdate,
        Credentials, HostDataBase, OfferRelay, Relayed, no_credentials,
    };
    ...
    const MAX_BITRATE_MBPS: f64 = 200.0;
    const MAX_VIDEO_WIDTH: u32 = 7680;
    const MAX_VIDEO_HEIGHT: u32 = 4320;
    const MAX_VIDEO_FPS: u32 = 240;
    const MAX_DETAIL_ROWS: u32 = 4096;
    const MAX_AUDIO_KBPS: u32 = 512;
    ...
    /// Do not publish the host's raw `/etc/machine-id` to the signaling
    /// service.  It is a stable system identifier and is unnecessarily useful
    /// for cross-application tracking.  An explicit operator value remains
    /// available for installations that already have an inventory identifier;
    /// otherwise expose only a stable one-way digest.
    fn privacy_device_id() -> String {
        if let Ok(configured) = std::env::var("OPENSTREAM_DEVICE_ID") {
            let configured = configured.trim();
            if !configured.is_empty() && configured.len() <= 128 {
    ...
    async fn run() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let configured_stun =
            std::env::var("LOWLAT_STUN").unwrap_or_else(|_| DEFAULT_STUN.to_string());
            ...
            .filter(|t| !t.is_empty())
        {
            ...
            if found.is_empty() {
                lowlat_common::log_warn!(
                    "lowlatd: reflexive server did not resolve, skipped: {name}"
                );
                continue;
            }
            for addr in found {
        ...
        let bitrate_mbps: f64 = flag("--bitrate")
            .and_then(|v| v.parse().ok())
            .filter(|value: &f64| value.is_finite() && *value > 0.0 && *value <= MAX_BITRATE_MBPS)
        ...
        let mut seam = Admission::new(Config {
            ...
            stream: Some(lowlat::stream::Config {
                ...
                audio_kbps: flag("--audio-kbps")
                    ...
                    .filter(|value| *value > 0 && *value <= MAX_AUDIO_KBPS)
