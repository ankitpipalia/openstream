...
fn a_session_round_does_not_allocate() {
    ...
    let mut guest_recv_bodies = vec![0u8; SLOT * SLOTS];
    let mut guest_recv_meta = vec![SlotMeta::default(); SLOTS];
    let mut guest_send_bodies = vec![0u8; SLOT * SLOTS];
    let mut guest_send_meta = vec![SendSlot::default(); SLOTS];
    ...
    session
        ...
        .unwrap();
    // The return leg needs the opposite direction bit: a session rejects its
    // own records, so the loop runs host -> guest -> host like the shell.
    let mut guest = Session::new_guest(Envelope::from_key(&KEY).unwrap(), 1, 0.0);
    guest
        .attach_recv(
            CHANNEL,
            RecvRing::new(&mut guest_recv_bodies, &mut guest_recv_meta, SLOT).unwrap(),
        )
        .unwrap();
    guest
        .attach_send(
            CHANNEL,
            SendRing::new(&mut guest_send_bodies, &mut guest_send_meta, SLOT, CHANNEL).unwrap(),
        )
        .unwrap();
    ...
    alloc_counter::assert_no_alloc(|| {
        for round in 1..32u32 {
            ...
            while let Some(result) = guest.get_output(now, &mut wire) {
                let written = result.unwrap();
                session
                    .process_input(&wire[..written], now, &mut scratch)
                    .unwrap();
            ...
            session.poll(now);
            guest.poll(now);
            while let Some(result) = guest.take_message(CHANNEL, &mut message) {
                std::hint::black_box(result.unwrap());
            }
        }
    });
