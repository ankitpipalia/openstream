...
/// Which side of a session sealed a record.
///
/// The bit rides in the most significant bit of the transmitted 64-bit
/// counter, so it is covered by the AES-GCM nonce (the nonce *is* the
/// prefix + counter). Both directions share one key, but a (key, nonce)
/// pair can never repeat across directions: the host seals with bit 0 and
/// opens with bit 1 expected, the guest mirrors it.
///
/// Interop note: a peer that does not implement the direction bit always
/// sends bit 0. A host-side session therefore rejects bit-0 records from
/// guests. Out-of-repo peers must implement the same rule.
...
impl Direction {
    /// The direction bit this side seals with, placed at counter bit 63.
    pub(crate) const fn seal_bit(self) -> u64 {
        (self as u64) << 63
    }
    ...
    pub const fn opposite(self) -> Direction {
        match self {
            Direction::Host => Direction::Guest,
            Direction::Guest => Direction::Host,
        }
    }

    /// Read the direction bit from a received datagram's counter field.
    /// Returns `None` when the datagram is too short to contain a counter.
    pub fn from_datagram(datagram: &[u8]) -> Option<Direction> {
        let counter = datagram.get(NONCE_OFFSET..NONCE_OFFSET + 8)?;
        if counter.first().copied()? & 0x80 == 0 {
            Some(Direction::Host)
        } else {
...
/// Direction separation comes from [`Direction`]: the seal side sets the
/// counter's top bit before sealing, so the AES-GCM nonce differs per
/// direction even though the key is shared.
pub struct Envelope {
    keyed: Keyed,
