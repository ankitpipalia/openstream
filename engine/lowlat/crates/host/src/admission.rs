...
/// Candidate trickle is untrusted input. Keep both the pre-approval buffer and
/// the live delivery queue bounded so a peer cannot grow host memory by sending
/// candidates faster than the connectivity loop consumes them.
const MAX_PENDING_CANDIDATES: usize = 64;

/// Application commands must not become an unbounded memory sink when a
/// guest's control path is stalled. A full queue is reported to the caller so
/// it can retry, coalesce, or drop according to its own policy.
const MAX_APPLICATION_MESSAGES: usize = 64;
...
pub enum Error {
    ...
    AlreadyBegun,
    /// An offer reused an identifier that is still active.
    DuplicateAttempt,
...
impl core::fmt::Display for Error {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let text = match self {
            ...
            Self::DuplicateAttempt => "attempt identifier is already active",
...
struct Attempt {
    ...
    guest: Option<Guest>,
    inject: Option<mpsc::SyncSender<SocketAddr>>,
...
impl Admission {
    ...
    pub fn add_candidate(&mut self, id: &str, addr: SocketAddr, sync: bool) {
        ...
        match (&attempt.inject, &attempt.guest) {
            ...
            _ if attempt.pending.len() < MAX_PENDING_CANDIDATES
                && !attempt.pending.contains(&addr) =>
            {
                attempt.pending.push(addr)
            }
            _ => {}
        }
    }
    ...
    pub fn begin_p2p(&mut self, id: &str, port: u16) -> Result<HostCredentials, Error> {
        ...
        let (inject, arrivals) = mpsc::sync_channel::<SocketAddr>(MAX_PENDING_CANDIDATES);
        for addr in attempt.pending.drain(..) {
            let _ = inject.try_send(addr);
        }
        let (say, said) = mpsc::sync_channel::<Said>(MAX_APPLICATION_MESSAGES);
        let (ask, asked) = mpsc::sync_channel::<Ask>(MAX_APPLICATION_MESSAGES);
    ...
    pub fn kick_guest(&mut self, guest: u32, reason: i32) -> bool {
        ...
        attempt
            .ask
            .as_ref()
            .is_some_and(|ask| ask.try_send(Ask::Kick(reason)).is_ok())
    }
    ...
    pub fn set_permissions(
        ...
        let sent = attempt
            .ask
            .as_ref()
            .is_some_and(|ask| ask.try_send(Ask::Permissions(permissions)).is_ok());
        if sent {
...
mod tests {
    ...
    #[test]
    fn duplicate_attempt_ids_cannot_replace_a_live_attempt() {
        let mut seam = admission(2);
        seam.new_attempt("same", peer())
            .expect("register first offer");
        seam.add_candidate("same", "203.0.113.9:41000".parse().unwrap(), false);
        assert_eq!(
            seam.new_attempt("same", peer()).unwrap_err(),
            Error::DuplicateAttempt
        );
        assert_eq!(seam.attempts["same"].pending.len(), 1);
    }
    ...
    fn candidate_trickle_is_deduplicated_and_bounded() {
        ...
        seam.add_candidate("bounded", "203.0.113.9:40000".parse().unwrap(), false);
        assert_eq!(
            seam.attempts["bounded"].pending.len(),
...
mod geometry {
    ...
    impl Arena {
        ...
        fn session(&mut self) -> Session<'_> {
            let envelope = Envelope::from_key(&KEY).expect("envelope");
            // Guest direction: seals with the guest bit, matching run_guest.
            let mut session = Session::new_guest(envelope, 1, 0.0);
            assert!(attach_recv(
                &mut session,
                CONTROL_CHANNEL,
                &mut self.control_recv_bodies,
