...
fn main() {
    eprintln!("desktop-probe is only available on Linux");
}
...
mod linux {
    ...
    fn main() {
        ...
        let mut file =
            std::fs::File::create(&out).unwrap_or_else(|e| fail(&format!("create: {e}")));
        ...
        if sizes.len() > 4 && identical + 4 >= sizes.len() {
            println!(
                "SUSPECT: the picture is barely changing; is the source re-reading the display?"
            );
        }
    }
