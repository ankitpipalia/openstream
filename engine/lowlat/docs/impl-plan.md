# Imported lowlat implementation plan

This plan belongs to the imported lowlat compatibility engine and remains
useful as its internal engineering record. It is not the current OpenStream
application plan. In particular, its Phase 2b direct/RFC 5766 relay work is
separate from the project-owned `openstream-client-core` path, which now has
optional `webrtc-ice` full ICE/TURN support. Use the repository-root
[`docs/IMPLEMENTATION_PLAN.md`](../../../docs/IMPLEMENTATION_PLAN.md) for
OpenStream status and release gates.
