...
**Status:** imported lowlat-engine design, locked 2026-08-15. Implemented by
`lowlat-core` (state machines) and `lowlat-net` (sockets), per
[00-overview.md](00-overview.md) D4. This document is not the specification
for the project-owned OpenStream peer session; see the repository-root
`docs/OPENSTREAM_PROTOCOL.md` for its optional standards-based ICE/TURN path.
...
This imported engine intentionally does **not** implement full ICE. Its
protocol uses ICE-shaped credentials and a candidate exchange, but the
procedure is a direct hole punch with authenticated binding checks, without
ICE's nomination, priority ordering, or role conflict resolution. That is a
valid design for this isolated sans-IO engine; it is not a claim that the
OpenStream application cannot use full ICE. `openstream-client-core` provides
that separate path through `webrtc-ice` when `OPENSTREAM_ICE=1` or
`OPENSTREAM_ICE_URLS` is configured.
